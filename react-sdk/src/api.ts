...
export async function fetchAdFromApi(
  sessionId: string,
  slot: string,
  position: number,
  signal?: AbortSignal
  ...
  if (!res.ok) throw new Error(`Ad fetch failed: ${res.status}`);
  const aid = res.headers.get('aid');
  if (!aid) throw new Error('Missing aid header');
  const html = await res.text();
  return { aid, html };
}

export async function trackImpression(aid: string, apiKey: string): Promise<void> {
